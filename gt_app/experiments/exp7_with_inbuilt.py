...
source = 'A'
all_distances, all_paths = nx.single_source_dijkstra(G, source)
...
for i, target in enumerate(targets):
    ...
    node_colors = ['skyblue' if node in path else '#eeeeee' for node in G.nodes()]
    ...
    ax.set_title(f"Path: {source} to {target}\nTotal Distance: {dist}", fontsize=11)
    ax.axis('off')
